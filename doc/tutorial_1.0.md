...
The fundamental concept in DENOPTIM's world is that chemical objects, such as molecules and materials, are made of building blocks that can be attached to form a **Graph**-like structure. Building blocks are, therefore, called **Vertices** or **Nodes** of the graph that defines a chemical object. The most intuitive type of building block is a **molecular fragment**. Molecular fragments can contain one or more atoms, bonds, and are decorated with **attachment points**. Attachment points collect define how a molecular fragment can be used to build larger chemical objects by attaching fragments to each other.
